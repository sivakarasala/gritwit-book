...
Throughout this book, we are going to write a program called **GrindIt** — a fitness tracking app. It will let you create exercises, log workouts, and track your progress. But first, we need to set up the workshop where we will build it.
...
This **creates a new folder** called `my_workouts` in whatever folder you are currently in. You will not see any output — silence means success. Run `ls` afterwards to confirm the folder was created.
...
Let's create the workspace where all of our GrindIt projects will live. Type each command one at a time:
...
| `.html` | An **HTML file** — the language of web pages | `index.html` |
| `.css` / `.scss` | **Stylesheets** — files that control how a web page looks (colors, fonts, layout) | `style.scss` |

You don't need to memorize these. You will get familiar with them naturally as we build GrindIt.
...
You just created a file and read it back — entirely from the command line.
...
5. Edit the file. Change it to say: `Hello GrindIt! I'm ready to learn Rust.`
...
```
Hello GrindIt! I'm ready to learn Rust.
```
